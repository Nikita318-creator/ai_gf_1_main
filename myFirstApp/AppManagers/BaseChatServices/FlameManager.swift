...
class FlameManager {
    ...
    @discardableResult
    func checkAndUpdateStreak(for chatID: String) -> FlameType? {
        ...
        let streak = realm.object(ofType: FlameObject.self, forPrimaryKey: chatID)
                     ?? FlameObject(chatID: chatID, count: 0, lastUpdateDate: nil)

        do {
            return try realm.write {
                var currentStreakType: FlameType?
    ...
    func getStreakCount(for chatID: String) -> Int {
        ...
        return realm.object(ofType: FlameObject.self, forPrimaryKey: chatID)?.count ?? 0
    }
}
